...
export const createStation = async (formData) => {
  try {
    ...
    if (stationId) {
      await query(
        "UPDATE stations SET isDefault = ?, isActive = ?, metaPreset = ?, name = ?, refUrl = ?, url = ?, donateLink = ?, gtm = ?, analytics = ?, advertisements = ?, logo = ?, thumbnail = ?, backgroundImage = ?, homepage = ?, email = ?, telephone = ?, address = ?, location = ? WHERE id = ?",
        [
          station.isDefault,
          station.isActive,
          station.metaPreset,
      ...
      const results = await query(
        "INSERT INTO stations (isDefault, isActive, metaPreset, name, refUrl, url, donateLink, gtm, analytics, advertisements, logo, thumbnail, backgroundImage, homepage, email, telephone, address, location) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        [
          station.isDefault,
          station.isActive,
          station.metaPreset,
