...
export const createStation = async (formData) => {
  try {
    ...
    const station = {
      ...
      callLetters: formData.get("callLetters"),
    };
    ...
    if (stationId) {
      await query(
        "UPDATE stations SET isDefault = ?, isActive = ?, metaPreset = ?, name = ?, refUrl = ?, url = ?, donateLink = ?, gtm = ?, analytics = ?, advertisements = ?, logo = ?, thumbnail = ?, backgroundImage = ?, homepage = ?, email = ?, telephone = ?, address = ?, location = ?, callLetters = ? WHERE id = ?",
        [
          station.isDefault,
          station.isActive,
          station.metaPreset,
      ...
      const results = await query(
        "INSERT INTO stations (isDefault, isActive, metaPreset, name, refUrl, url, donateLink, gtm, analytics, advertisements, logo, thumbnail, backgroundImage, homepage, email, telephone, address, location, callLetters) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)",
        [
          station.isDefault,
          station.isActive,
          station.metaPreset,
